...
import { Menu, X, Sparkles, Activity, Lock, Unlock, KeyRound } from 'lucide-react';
import { motion as Motion, AnimatePresence } from 'framer-motion';
import ThemeToggle from './ThemeToggle';
import LanguageToggle from './LanguageToggle';
import { portfolioData } from '../data/portfolio';

const Nav = ({ theme, setTheme, language, setLanguage, isAdmin, setIsAdmin }) => {
    ...
    const handleUnlockSubmit = (e) => {
        e.preventDefault();
        if (!passInput.trim()) {
            setPassError(language === 'vi' ? 'Vui lòng nhập mật khẩu' : 'Please enter password');
            return;
        }
        localStorage.setItem("adminToken", passInput.trim());
        setIsAdmin(true);
    ...
    const handleLock = () => {
        ...
        setIsAdmin(false);
        if (location.pathname === '/monitor' || location.pathname === '/chatbot') {
            navigate('/');
        }
    };
    ...
    return (
        <>
                ...
                <div className="mx-auto max-w-6xl px-4 sm:px-6">
                    <div className={`flex items-center justify-between rounded-3xl glass-3d-card px-6 py-3.5 transition-all duration-500 ${
                        ...
                        <nav className="hidden md:flex items-center space-x-1.5">
                            ...
                            {/* Protected AI Chatbot Link (Only visible when unlocked) */}
                            {isAdmin && (
                                <Link 
                                    to="/chatbot" 
                            ...
                            {/* Protected Monitor Link (Only visible when unlocked) */}
                            {isAdmin && (
                                <Link 
                                    to="/monitor" 
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            {isAdmin ? (
                                <button
                                    onClick={handleLock}
                                    title={language === 'vi' ? 'Khóa tính năng Admin' : 'Lock Admin Features'}
                                    className="p-2 rounded-2xl glass-3d-card text-emerald-500 hover:text-rose-500 transition-colors border border-emerald-500/30"
                                >
                                    <Unlock size={16} />
                                </button>
                            ) : (
                                <button
                                    ...
                                    title={language === 'vi' ? 'Nhập mật khẩu mở khóa Admin' : 'Unlock Admin Mode'}
                                    className="p-2 rounded-2xl glass-3d-card text-slate-500 dark:text-slate-400 hover:text-indigo-500 transition-colors border border-slate-300/40 dark:border-slate-700/40"
                                >
                                    <Lock size={16} />
                                </button>
                            )}
                            ...
                            <div className="rounded-3xl glass-3d-card p-5 shadow-2xl flex flex-col space-y-2 border border-slate-200/80 dark:border-slate-800/80">
                                ...
                                {isAdmin && (
                                    <Link 
                                        to="/monitor" 
                                        onClick={() => setIsOpen(false)}
                                        className="px-4 py-3 text-sm font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 rounded-2xl flex items-center gap-2"
                                    >
                                        <Activity size={16} />
                                        <span>Monitor</span>
                                    </Link>
            ...
            {/* 3D Glassmorphic Password Modal */}
            <AnimatePresence>
                {showAuthModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
                        <Motion.div
                            ...
                            className="relative w-full max-w-md p-8 glass-3d-card rounded-3xl border border-white/20 dark:border-slate-800/80 shadow-2xl"
                        >
                            <button
                            ...
                            <div className="flex flex-col items-center text-center space-y-4">
                                <div className="p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-500">
                                    <KeyRound size={32} />
                                </div>
                                <h3 className="text-xl font-bold text-slate-900 dark:text-white">
                                    {language === 'vi' ? 'Mở khóa Tính năng Admin' : 'Unlock Admin Features'}
                                </h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                    {language === 'vi' 
                                        ? 'Nhập mật khẩu Admin để hiển thị AI Assistant ChatBot và Dashboard Monitor.' 
                                        : 'Enter Admin password to unlock AI Assistant ChatBot and System Monitor Dashboard.'}
                                </p>

                                <form onSubmit={handleUnlockSubmit} className="w-full space-y-4 pt-2">
                                    <div>
                                        <input
                                            type="password"
                                            value={passInput}
                                            onChange={(e) => setPassInput(e.target.value)}
                                            placeholder={language === 'vi' ? 'Nhập mật khẩu Admin...' : 'Enter Admin password...'}
                                            className="w-full px-4 py-3 text-sm rounded-2xl glass-3d-card border border-slate-300 dark:border-slate-700 focus:outline-none focus:border-indigo-500 text-slate-900 dark:text-white placeholder-slate-400"
                                            autoFocus
                                        />
                                        {passError && (
                                            <p className="text-xs text-rose-500 mt-1 text-left">{passError}</p>
                                        )}
                                    </div>

                                    <button
                                        type="submit"
                                        className="w-full py-3 px-6 rounded-2xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold text-sm shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all active:scale-98"
                                    >
